...
export default function ProfilePage() {
  ...
  const loadUserData = async () => {
    try {
      ...
      if (!error && data) {
        ...
        setUserStats(prev => ({ ...prev, isPro: data.is_pro || false }))
      } else {
  ...
  const loadUserStats = async () => {
    try {
      ...
      const totalTokensUsed = (tokensData || []).reduce((sum: number, msg: any) => sum + (msg.tokens_used || 0), 0)
  ...
  const handleSaveProfile = async () => {
    try {
      console.log('Сохранение профиля:', displayName)
      setIsEditing(false)
    } catch (error) {
        ...
        <div className="bg-background-secondary rounded-xl p-6 border border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center space-x-3 p-4 bg-background rounded-lg">
              ...
              <div>
                <p className="text-lg font-bold text-text-primary">{userStats.remainingTokens}</p>
                <p className="text-sm text-text-secondary">Осталось токенов</p>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-sm text-text-secondary">
                  {userStats.isPro ? 'PRO план' : 'Starter базовый план'}
                </p>
              </div>
