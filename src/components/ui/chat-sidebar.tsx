    ...
    <div className={`${isMobile ? 'w-full' : 'w-80'} bg-background-secondary border-r border-border flex flex-col h-full`}>
      ...
      {isMobile && (
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-lg font-semibold text-text-primary">AI Chat Pro</h2>
          <button
            onClick={onClose}
      ...
      {!isMobile && (
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-text-primary">AI Chat Pro</h2>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={onUpgrade}
              className="px-4 py-2 bg-primary text-white rounded-lg font-semibold text-sm flex items-center space-x-2 hover:bg-primary-dark transition-colors cursor-pointer"
            >
        ...
        <div className="mb-6">
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className="text-lg font-bold text-primary">{userTokens.toLocaleString()}</p>
              <p className="text-sm text-text-secondary">
                {isPro ? 'Неограниченно' : 'Осталось'}
              </p>
            </div>
            <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
